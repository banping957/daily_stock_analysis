...
1. **当前是否适合做网格交易？**
   - 判断标准：
     * 近 30 日波动率 > 8%，且无明显单边趋势（震荡市）
     * 有明确的支撑位和压力位
     * 不处于强势上涨或下跌阶段
   - 如果处于明显的单边趋势，明确标注 **"❌ 不适合网格交易"**

2. **建议的网格区间**（仅当适合时）：
   - **下界**：支撑位/近期低点（可参考 MA20, MA60 或历史底部）
   - **上界**：压力位/近期高点（可参考前高、整数关口）
   - **当前价格**在该区间的位置（上部/中部/下部）

3. **建议网格参数**（仅当适合时）：
   - 网格数量：5-10 档
   - 每格利润率：建议 3-5%（根据波动率调整）
   - 总仓位建议：分批建仓，首仓 20-30%

4. **风险提示**：
   - 如果出现重大利空或破位下跌，网格策略失效
   - 必须设置止损位（如跌破 MA60 或区间下界）
...
✅ **适合网格交易**
- 网格区间：80.00 ~ 100.00 元
- 当前价格：90.00 元（区间中部，位置适中）
- 建议网格：10 档，每档 2.00 元，利润率约 2.2%
- 止损位：75.00 元（跌破 MA60）
- 理由：近期在区间内震荡，波动率适中，支撑压力明确
```

**如果不适合网格交易**：
```
❌ **不适合网格交易**
- 原因：当前处于强势上涨趋势，均线多头排列，不宜设置网格
- 建议：等待回调到支撑位后再考虑，或采用趋势跟踪策略
```

请在决策仪表盘的 `dashboard.battle_plan` 部分新增 `grid_trading` 字段包含此分析。
